...
const shapey = screen.find("Shapey 2");
...
if (window.DeviceMotionEvent && window.DeviceMotionEvent.requestPermission) {
    // On iphones we need to have premission
    window.DeviceMotionEvent.requestPermission().then(function(response) {
        if (response == 'granted') {
            VanillaTilt.init(shapey.node, vanillaTiltOptions);
        } else {
            alert('Disallowed access. Make sure you\'re on HTTPS and you cleared cookies before trying again.');
        }
    });   
} else {
    VanillaTilt.init(shapey.node, vanillaTiltOptions);
